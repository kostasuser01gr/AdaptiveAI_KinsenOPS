...
export default function Sidebar() {
  ...
  const { data: notificationsData } = useQuery({ queryKey: ["/api/notifications"], enabled: !!user });
  const { data: dashStats } = useQuery<{ vehicles: number; washQueue: number; shifts: number; users: number; stations: number; automations: number; warRooms: number; unreadNotifications: number; pendingShiftRequests: number }>({ queryKey: ["/api/dashboard-stats"], enabled: !!user, refetchInterval: 30000 });
    ...
    <div className={`fixed inset-y-0 left-0 z-40 w-[280px] bg-sidebar flex flex-col transition-transform duration-300 ease-in-out transform border-r border-sidebar-border ${
      ...
      <ScrollArea className="flex-1 px-3">
        <div className="py-2 flex flex-col gap-1">
          <Button variant={location === '/' ? 'secondary' : 'ghost'} className="w-full justify-start h-10 px-3 rounded-lg group" asChild>
            <Link href="/">
              <div className="flex items-center gap-3 w-full">
                <Bot className="h-4 w-4" />
                <span className="font-medium text-sm flex-1">{t('new_chat')}</span>
                <Plus className="h-4 w-4 opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
            </Link>
          </Button>
          <Button variant={isModuleActive('/inbox') ? 'secondary' : 'ghost'} className="w-full justify-start h-10 px-3 rounded-lg" asChild>
            <Link href="/inbox">
              <div className="flex items-center gap-3 w-full">
                <Inbox className="h-4 w-4" />
                <span className="font-medium text-sm flex-1">{t('ops_inbox')}</span>
                {unreadCount > 0 && <Badge variant="destructive" className="h-5 px-1.5 text-[10px]" data-testid="badge-unread-count">{unreadCount}</Badge>}
              </div>
            </Link>
          </Button>
        </div>
        ...
        <div className="flex flex-col gap-0.5 pb-4">
          {pinnedConversations.length > 0 ? (
            ...
            <p className="px-3 text-xs text-sidebar-foreground/40 italic">No pinned threads yet</p>
          )}
        </div>
      </ScrollArea>
      ...
      <div className="p-3 border-t border-sidebar-border bg-sidebar">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="w-[250px] mb-2">
            ...
            <DropdownMenuItem className="cursor-pointer py-2 text-primary" onClick={() => window.open('/washer', '_blank')}>Launch Washer Kiosk</DropdownMenuItem>
            <DropdownMenuItem className="cursor-pointer py-2 text-primary" onClick={() => window.open('/customer', '_blank')}>Launch Customer Portal</DropdownMenuItem>
            <DropdownMenuSeparator />
